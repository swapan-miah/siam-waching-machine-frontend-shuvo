...
export default function Page() {
	...
	const fetchData = async () => {
		try {
			const res = await fetch(`${process.env.BASE_URL}/why-choose`);
	...
	return (
		<div className="p-5">
			<div className="flex items-center justify-between mb-3">
				...
				<Link href={`/dashboard/choose-us/edit/${data._id}`} onClick={() => setLoading(true)}>
				...
				{data.expertise.map((expert, idx) => (
					<div
						...
						className={`p-4 bg-white rounded-lg border transition-all duration-300 ease-in-out cursor-pointer ${
							openIndex === `expertise-${idx}` ? "border-gray-400" : "border-gray-200"
						}`}
					>
						<h3
						...
						<div
							className={`transition-all overflow-hidden duration-300 ease-in-out ${
								openIndex === `expertise-${idx}` ? "max-h-[1000px] mt-4" : "max-h-0"
							}`}
						>
							<div className="mt-4 space-y-4">
								...
								<div className="bg-gray-50 p-3 rounded-lg border">
									...
									<p className="text-gray-600 mt-2">{expert.expertiseDescription}</p>
								</div>
							</div>
						</div>
					</div>
