...
export default function Page() {
    ...
    const fetchData = async () => {
        try {
            const res = await fetch(`${process.env.BASE_URL}/seo/services`);
            if (!res.ok) {
	...
	const handleDelete = async () => {
		setLoading(true);
		try {
			const res = await axios.delete(`${process.env.BASE_URL}/seo/services`);
		...
		return (
			<div className="p-5 h-full f-full">
                <div className="flex items-center justify-between mb-3">
                    ...
                    {!data && (
                        <Link href="/dashboard/seo/services/create">
