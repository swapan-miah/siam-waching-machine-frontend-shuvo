...
export default function page() {
	...
	const fetchData = async () => {
		try {
			const res = await fetch(`${process.env.BASE_URL}/services-skills`);
		...
		<div className="p-5">
			...
			<div className="overflow-x-auto">
				<table className="hidden min-w-full lg:table border border-gray-200">
					...
					<tr>
						<th className="px-4 py-2 text-left text-sm font-semibold uppercase">Photo</th>
					...
					{[...data].reverse().map((sData) => (
						...
						<td className="px-4 py-3 text-sm"><span className="line-clamp-1 break-all">{sData.title}</span></td>
						<td className="px-4 py-3 text-sm"><span className="line-clamp-2 break-all">{getFirstTagText(sData.description)}</span></td>
				...
				<div className="lg:hidden space-y-4 mt-4">
					...
					<div key={sData._id} className="divide-y divide-gray-100 bg-white border border-gray-200 rounded-lg">
						...
						<div className="flex justify-between gap-3 px-3 py-4">
							...
							<span className="text-md text-gray-900 line-clamp-1">{sData.title}</span>
						</div>

						<div className="flex justify-between gap-3 px-3 py-4">
							<span className="text-md font-semibold text-gray-700">Description:</span>
							<span className="text-md text-gray-900 line-clamp-2">{getFirstTagText(sData.description)}</span>
						</div>
